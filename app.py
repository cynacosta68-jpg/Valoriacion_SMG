    ...
    if uploaded_file_valorizacion is not None:
        ...
        try:
            ...
            if not df_evweb.empty:
                ...
                st.subheader("DataFrame con nuevas columnas añadidas:")
                st.dataframe(df_final.head())
                st.write(f"Columnas del DataFrame final: {df_final.columns.tolist()}")
...
if 'df_final' in st.session_state and st.session_state['df_final'] is not None and \
   'uploaded_file_valorizacion_object' in st.session_state and st.session_state['uploaded_file_valorizacion_object'] is not None:
    ...
    def aplicar_valorizacion_definitiva_streamlit(df_f, uploaded_valorizacion_file_obj):
        ...
        # Handle potential missing 'Mes' or 'Periodo' columns
        if 'Mes' in df_uni.columns and not df_uni['Mes'].empty:
            df_uni['periodo_aux'] = pd.to_datetime(df_uni['Mes'], errors='coerce').dt.to_period('M')
        else:
            df_uni['periodo_aux'] = pd.NaT
            st.warning("La columna 'Mes' no se encontró o está vacía en la hoja 'unidades'. La valorización podría verse afectada.")

        if 'Periodo' in df_fijos.columns and not df_fijos['Periodo'].empty:
            df_fijos['periodo_aux'] = pd.to_datetime(df_fijos['Periodo'], errors='coerce').dt.to_period('M')
        else:
            df_fijos['periodo_aux'] = pd.NaT
            st.warning("La columna 'Periodo' no se encontró o está vacía en la hoja 'Valor Fijos'. La valorización podría verse afectada.")
        ...
        def calcular_total(row):
            try:
                ...
                if isinstance(importe_val, str) and "#REVISAR VALORES" in importe_val:
                    return importe_val
